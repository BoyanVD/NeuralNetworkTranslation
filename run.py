...
if len(sys.argv)>1 and (sys.argv[1] == 'train' or sys.argv[1] == 'extratrain'):
    ...
    encoder = model.Encoder(len(sourceWord2ind), EMBEDDING_SIZE, HIDDEN_SIZE, NUMBER_OF_LSTM_LAYERS, DROPOUT)
    decoder = model.Decoder(len(targetWord2ind), EMBEDDING_SIZE, HIDDEN_SIZE, len(targetWord2ind), NUMBER_OF_LSTM_LAYERS, DROPOUT)
    nmt = model.NMTmodel(encoder, decoder, sourceWord2ind, targetWord2ind, unkToken, padToken, endToken, startToken).to(device)
